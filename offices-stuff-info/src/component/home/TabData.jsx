        ...
        <div className="table-responsive">
            ...
            <tbody>
              {currentRows.map(row => (
                <tr key={row._id} className="text-center">
                  ...
                  <td>
                    {row.platform}{" "}
                    {row.callMethod === "incoming" && <FiPhoneIncoming />}
                    {row.callMethod === "outgoing" && <FiPhoneOutgoing />}
                  </td>
                  <td>{row.comments}</td>
                </tr>
              ))}
          ...
          <div className="pagination-container">
            <Pagination>
              ...
              {Array.from({ length: Math.ceil(filteredRows.length / rowsPerPage) }, (_, index) => (
                <Pagination.Item
                  key={index + 1}
              ...
              <Pagination.Next onClick={handleNext} disabled={currentPage === Math.ceil(filteredRows.length / rowsPerPage)} />
            </Pagination>
          </div>
        </div>
      </div>
